...
async def process_request(request: RequestModel, provider: Dict):
    ...
    # 如果接口名称里有 chat/completions，则使用 gpt 引擎
    ...
    request_info = {"url": url, "headers": headers, "payload": payload}
    logger.info(
        f"Request details: {json.dumps(request_info, indent=4, ensure_ascii=False)}"
    )
...
class ModelRequestHandler:
    ...
    def get_matching_providers(self, model_name, token):
        ...
        logger.info(f"get_matching_providers model_name: {model_name}")
        logger.info(f"get_matching_providers token: {token}")
        logger.info(f"get_matching_providers api_list: {api_list}")

        api_index = api_list.index(token)
        provider_rules = []

        for model in config["api_keys"][api_index]["model"]:
            logger.info(f"get_matching_providers model: {model}")

            # 如果 model 是 *，则直接返回所有 provider 下面的所有 model 的集合列表
            if model == "*":
                for provider in config["providers"]:
                    for model_ in provider["model"].keys():
                ...
                # print("model_", model_)
                if (model_ and model_name in models_list) or (
                    model_ == "*" and model_name in models_list
                ):
    ...
    async def request_model(self, request: RequestModel, token: str):
        ...
        logger.info(f"request_model model_name: {model_name}")
        logger.info(
            "request_model matching_providers: %s",
            matching_providers if matching_providers else "No matching model found",
        )
        ...
        if config["api_keys"][api_index].get("preferences"):
            use_round_robin = config["api_keys"][api_index]["preferences"].get(
                "USE_ROUND_ROBIN"
            )
            auto_retry = config["api_keys"][api_index]["preferences"].get("AUTO_RETRY")

        return await self.try_all_providers(
            request, matching_providers, use_round_robin, auto_retry
        )
        ...
        for i in range(num_providers + 1):
            ...
            try:
                ...
                if auto_retry:
                    ...
                    raise HTTPException(
                        status_code=500,
                        detail="Error: Current provider response failed!",
                    )

        raise HTTPException(
            status_code=500, detail=f"All providers failed: {request.model}"
        )
...
async def request_model(request: RequestModel, token: str = Depends(verify_api_key)):
    # logger.info(f"request_model: {request}")
    # 先关掉 stream
